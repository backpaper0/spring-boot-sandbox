...
class CustomEventListenerFactory implements EventListenerFactory, Ordered, ApplicationContextAware {
    ...
    public ApplicationListener<?> createApplicationListener(
            final String beanName, final Class<?> type, final Method method) {
        if (method.isAnnotationPresent(FooListener.class)) {
            return new CustomApplicationListener(applicationContext, beanName, method, SampleEvent.FOO);
        }
        if (method.isAnnotationPresent(BarListener.class)) {
            return new CustomApplicationListener(applicationContext, beanName, method, SampleEvent.BAR);
        }
        return new CustomApplicationListener(applicationContext, beanName, method, SampleEvent.values());
    }
