...
public class SongControllerMockMvcTest extends RestControllerTestBase {
    ...
    void testGetListWithJSONAssert() throws Exception {
        ...
        JSONAssert.assertEquals(read("expected-list.json"), response, false);
    }
    ...
    void testCreate() throws Exception {
        String response = mvc.perform(
                        post("/songs").contentType(MediaType.APPLICATION_JSON).content(read("create.json")))
                .andExpectAll(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(StandardCharsets.UTF_8);

        JSONAssert.assertEquals(read("expected-create.json"), response, false);
    }
}
